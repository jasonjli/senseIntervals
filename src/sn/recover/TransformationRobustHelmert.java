...
//        str.append(formatPrecise(scale < 1 ? 1. / scale : scale));
//        str.append(" +/-");
//        final double scaleSigma = this.getScaleSigma(invert);
//        str.append(formatPreciseShort(scale < 1 ? scale / scaleSigma : scaleSigma));
//        str.append(nl);
//
...
//        str.append(formatPreciseShort(Math.toDegrees(this.getRotationSigma())));
        ...
        str.append(getBreakLineForReport());
        str.append("Standard deviation and mean position error for all points:" + nl + nl);
...
//        str.append(nl);
    ...
    private Matrix constructP() {
        if (this.mat_v == null) {
            ...
            final double dmed = getMedian(d); //Median.median(d, true);
            ...
            this.s = getMedian(d_minus_dmed) / 0.4485; //Median.median(d_minus_dmed, false) / 0.4485;
    ...
    protected void initWithPoints(double[][] destSet, double[][] sourceSet) {
        ...
        final double mad = getMedian(v_abs); // Median.median(v_abs, false);
    ...
    public double getMedian(double[] numArray){
    	double[] sortedArray = numArray.clone();
    	Arrays.sort(sortedArray);
    	double median;
    	if (numArray.length % 2 == 0)
    	    median = ((double)sortedArray[sortedArray.length/2] + (double)sortedArray[numArray.length/2 - 1])/2;
    	else
    ...
    public Point2D transform(Point2D point){
    	double[] sourcePoint = new double[2];
    	sourcePoint[0] = point.getX();
    	sourcePoint[1] = point.getY();
    	double[] destPoint = transform(sourcePoint);
    	Point2D newPoint = new Point2D.Double(destPoint[0], destPoint[1]);
    	return newPoint;
    }
